...
<template>
    <div>
        ...
        <p class="text-white text-lg">A continuaciíon verifica la informacion y confirma tu cita</p>
        ...
        <div class="space-y-8" v-if="!appointments.noServicesSelected">
            ...
            <div class="lg:flex gap-5 items-start">
                <div class="m-full lg:w-96 bg-white flex justify-center rounded-lg">
                    <VueTailwindDatepicker i18n="es-CO" as-single no-input :formatter="formatter"
                        v-model="appointments.date" :disable-date="disableDate" />
                </div>

                <div v-if="appointments.isDateSelected" class="flex-1 grid grid-cols-1 xl:grid-cols-2 gap-5 mt-10 lg:mt-0">
                    <button v-for="hour in appointments.hours" class="block text-blue-500 rounded-lg text-xl font-black p-3 disabled:opacity-10"
                        :key="hour" :class="appointments.time === hour ? 'bg-blue-500 text-white' : 'bg-white'"
